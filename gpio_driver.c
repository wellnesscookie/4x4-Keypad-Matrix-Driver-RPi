...
int gpio_driver_init(void)
{
    ...
    SetGpioPinDirection(GPIO_17, GPIO_DIRECTION_OUT);
    
    // Setujte na LOW  (0) dati pin
    ...
    // Redovi R0 - R3
    SetGpioPinDirection(GPIO_18, GPIO_DIRECTION_IN);
    SetInternalPullUpDown(GPIO_18, PULL_DOWN);
    ...
    SetInternalPullUpDown(GPIO_24, PULL_DOWN);
    
    SetGpioPinDirection(GPIO_25, GPIO_DIRECTION_IN);
    SetInternalPullUpDown(GPIO_25, PULL_DOWN);
...
int rowPins[ROWS] = {GPIO_18, GPIO_23, GPIO_24, GPIO_25};
int colPins[COLS] = {GPIO_10, GPIO_22, GPIO_27, GPIO_17};
...
char* get_key(void)
{
   ...
   for (c = 0; c < COLS; c++)
   {
      ...
      for (r = 0; r < ROWS; r++)
      {
            ...
            if (GetGpioPinValue(rowPins[r]) == 1) 
            {
                    pressedKeys[j++] = keys[r][c];
            }     
      }   
      /* Deactivate column */
      udelay(12);
      ClearGpioPin(colPins[c]);
   }
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
    ...
    /* fill gpio_driver_buffer */
    memset(gpio_driver_buffer, 0, 16);
    
    /* call the logic to scan and get pressed keys */
    pressedKeys = get_key();
    
	sprintf(gpio_driver_buffer, "%s", pressedKeys);
